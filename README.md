...
Beim Speichern steht der kanonische Vorlagendateiname als erstes Feld vor `content`, beispielsweise `"filename": "Speisekarte.json"`. Das gilt fuer Vorlage und Live-Datei. Beim Laden der Live-Karte waehlt der Editor die dazu passende vorhandene Vorlage samt Archiv; Inhalte werden weiterhin aus der Live-Datei geladen. Ohne Metadatum oder bei fehlender Vorlage bleibt die Auswahl bei der aktuellen Karte. Ein expliziter Vorlagen- oder Archivwechsel hat Vorrang vor Metadaten aus alten Dateien. Auch das Speicherziel "Aktuelle Menuekarte" aktualisiert die zugeordnete Vorlage, sofern sie vorhanden ist; ohne Zuordnung wird nur `data.json` gespeichert. Alte Archive werden nicht nachtraeglich veraendert.

Jede Speisekarten-Vorlage hat ihre eigene Archivliste. Beim Aktualisieren wird ihr bisheriger Stand unter `templates/archiv/<Vorlagenname>_YYYY-MM-DD_HH-MM-SS_ffffff.json` gesichert (Zeitzone Europe/Berlin, Mikrosekunden mit Kollisionsschutz). Die aktuelle veroeffentlichte Karte hat ein separates `data_...`-Archiv. Eine neu angelegte Vorlage hat noch keinen vorherigen Stand und daher anfangs kein Archiv. Bestehende Archivnamen ohne Mikrosekunden werden weiterhin erkannt. Die Zuordnung verwendet den vollstaendigen Namen, sodass beispielsweise `Sommer` und `Sommer_Spezial` getrennt bleiben. Zuerst erscheinen die neuesten 20 Staende; weitere lassen sich nachladen.

Neue Vorlagennamen werden im Editor und beim Speichern serverseitig ohne Unterscheidung der Gross-/Kleinschreibung auf Duplikate geprueft. `data` ist reserviert. Namen duerfen Buchstaben, Zahlen, Leerzeichen, Bindestriche, Unterstriche, Punkte und Klammern enthalten, muessen mit einem Buchstaben oder einer Zahl beginnen und duerfen nicht mit einem Punkt enden (maximal 155 UTF-8-Bytes). Die API `save_file` erwartet fuer neue Vorlagen `create: true`; ein Namenskonflikt liefert HTTP 409. Ohne `create` wird eine bestehende Vorlage aktualisiert. Die Datei `templates/.save.lock` serialisiert Namenspruefung und Speicherung; der Webserver benoetigt dort Schreibrechte.

Der Speicherdialog waehlt die geladene Karte vor. Jede gespeicherte Vorlage wird gleichzeitig nach `speisekarte/data.json` uebernommen und damit live geschaltet. Der vorherige Vorlagenstand und die vorherige Live-Karte werden getrennt archiviert. Beide neuen Dateien werden vor der Uebernahme vorbereitet; bei einem Uebernahmefehler wird eine bereits ersetzte Vorlage zurueckgesetzt. Nach dem Speichern werden Vorlagen- und Archivliste aktualisiert. Das Laden eines Archivstands aendert nur den Editorinhalt und aktiviert das Speichern; erst das anschliessende Speichern stellt diesen Stand wieder her und schaltet ihn live. Die bisherigen Versionen werden dabei erneut archiviert.

Gezielter Regressionstest ohne Build und ohne Aenderung echter Inhaltsdateien: `node tools/test_menu_archives.js /pfad/zum/php`. Ohne Interpreterargument wird `php` aus dem PATH verwendet. Der Test nutzt eine temporaere Kopie des API-Handlers und entfernt seine Testdaten anschliessend.
